import * as assert from 'assert';
import * as fs from 'fs';
import { OssService } from '../repository/oss';
import { DropBoxService } from '../repository/dropbox';
import { S3Service } from '../repository/s3';

/**
 * PLEASE upload 'test-data' to your repository before running test
 */

const repository = new OssService({
  name: 'repo',
  type: 'oss',
  options: {
    region: '...',
    accessKeyId: '...',
    accessKeySecret: '...',
    bucket: 'dbox-repo',
  },
});

// const repository = new DropBoxService({
//   name: 'repo',
//   type: 'dropbox',
//   options: {
//     accessToken:
//       '...',
//     root: 'dbox-repo',
//   },
// });

// const repository = new S3Service({
//   "name": "repo",
//   "type": "s3",
//   "options": {
//     "credentials": {
//       "accessKeyId": "...",
//       "secretAccessKey": "..."
//     },
//     "region": "...",
//     "bucket": "dbox-repo"
//   }
// });

describe('reposiotry service', async function() {
  this.timeout(6000);

  it('should load metadatas of extensions from a specific repository.', async () => {
    const metadatas = await repository.listExtensions();
    assert.deepStrictEqual(metadatas, [
      {
        extId: 'dteam.my-ext',
        currentVersion: '0.0.1',
        icon: '',
        describe: 'my-ext',
        dateCreated: '2020-02-28',
        lastUpdate: '2020-02-29',
      },
      {
        extId: 'dteam.another-ext',
        currentVersion: '0.0.2',
        icon:
          'data:image/png;base64,',
        describe: 'another-ext',
        dateCreated: '2020-02-28',
        lastUpdate: '2020-02-29',
      },
    ]);
  });

  it('should show metadata of an extension.', async () => {
    const metadata = await repository.showExtension('dteam.another-ext');
    assert.deepStrictEqual(metadata, {
      metadata: {
        extId: 'dteam.another-ext',
        currentVersion: '0.0.2',
        icon:
          'data:image/png;base64,',
        describe: 'another-ext',
        dateCreated: '2020-02-28',
        lastUpdate: '2020-02-29',
      },
      readme:
        '# another-ext README\n\nThis is another ext. And its version is 0.0.2\n',
      changelog:
        '# Another-Ext Change Log\n\n## 0.0.2\n\nHere we are.\n\n## 0.0.1\n\nAll notable changes to the "another-ext" extension will be documented in this file.\n\nCheck [Keep a Changelog](http://keepachangelog.com/) for recommendations on how to structure this file.\n\n## [Unreleased]\n\n- Initial release',
    });
  });

  it('should show metadata of an extension which has not readme or changelog.', async () => {
    const metadata = await repository.showExtension('dteam.my-ext');
    assert.ok(metadata);
  });

  it('should download an extension.', async () => {
    const result = await repository.downloadExtension('dteam.another-ext');
    assert.deepStrictEqual(result.metadata, {
      extId: 'dteam.another-ext',
      currentVersion: '0.0.2',
      icon:
        'data:image/png;base64,',
      describe: 'another-ext',
      dateCreated: '2020-02-28',
      lastUpdate: '2020-02-29',
    });
    assert.ok(fs.existsSync(result.vsixPath));
  });
});
